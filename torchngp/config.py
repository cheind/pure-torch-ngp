...
def load_transforms_json(paths: Union[str, list[str]]) -> SceneConf:
    """Loads scene information from NeRF transforms.json files.
...
def _load_transforms_json_data(path: str):
    ...
    camcfg = MultiViewCameraConf(
        focal_length=(fl_x, fl_y),
        principal_point=(cx, cy),
        size=(W, H),
        rvec=Vecs3Conf([tuple(r.tolist()) for r in rvec]),
        tvec=Vecs3Conf([tuple(t.tolist()) for t in tvec]),
